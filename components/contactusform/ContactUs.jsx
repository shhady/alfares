...
export default function ContactUs() {
  ...
  const handleSubmit = (e) => {
    ...
    const { budget, ready, name, phone, email, how } = formData;
    const whatsappMessage = `اسم: ${encodeURIComponent(name)}%0Aرقم الهاتف: ${encodeURIComponent(phone)}%0Aالبريد الإلكتروني: ${encodeURIComponent(email)}%0Aالميزانية: ${encodeURIComponent(budget)}%0Aجاهز للحجز: ${encodeURIComponent(ready)}%0Aكيف تعرف عنا: ${encodeURIComponent(how)}`;

    // Open WhatsApp with the message
    const whatsappUrl = `https://api.whatsapp.com/send?phone=972543113297&text=${whatsappMessage}`;
