...
export default function PropertyCard({property, showAs}) {
  ...
  return (
    <Link href={`/properties/${property._id}`}>
      <div className={`relative z-1 ${showAs === 'table' ? 'flex p-2 gap-2 border-2 border-black':''}`}>
      ...
      {property.images.length}<Camera size={16}/> 
       ...
       <Image
                            ...
                            alt="image"
                            className={`min-h-60 max-h-60 ${showAs === 'table' ? 'w-1/4':'w-full'} h-auto object-cover rounded-t-lg `}
                        />
